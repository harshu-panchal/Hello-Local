...
export interface AdminOrdersByStatusProps {
  /** The order status this page lists (e.g. 'Pending', 'Received', 'Processed', 'Shipped', 'Out for Delivery', 'Delivered', 'Cancelled'). */
  status: string;
  /** Slug used for the exported CSV filename. Defaults to the lowercased status. */
  exportSlug?: string;
}
...
export default function AdminOrdersByStatus({
  status: initialStatus,
  exportSlug,
}: AdminOrdersByStatusProps) {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const response = await getOrdersByStatus(status, params);
      ...
      const msg = err.response?.data?.message || `Failed to load ${status.toLowerCase()} orders`;
      setTableError(msg);
      showToast(msg, "error");
      setOrders([]);
    } finally {
  ...
  const handleExport = () => {
    ...
    const slug = exportSlug || status.toLowerCase().replace(/\s+/g, "_");
    link.setAttribute("download", `hellolocal_orders_${slug}_${new Date().toISOString().split("T")[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    showToast("Order records exported successfully", "success");
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
        <div>
          ...
          <p className="text-xs sm:text-sm text-neutral-500 mt-0.5">
            Real-time fulfillment queue, dispatch assignments, and lifecycle tracking for {status.toLowerCase()} orders
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-neutral-200/80 overflow-hidden">
        ...
        <div className="bg-rose-700 text-white px-5 py-3.5 flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-sm sm:text-base font-bold tracking-tight">
            {status} Orders Queue ({totalOrders} Total)
          </h2>
          <button
            type="button"
        ...
        <div className="p-4 border-b border-neutral-200/70 bg-neutral-50/50 space-y-3.5">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {/* From Date */}
            <div>
              <label htmlFor="orderStatusDateFrom" className="block text-[11px] font-bold text-neutral-700 mb-1 uppercase tracking-wider">
                From Date
              </label>
              <input
                id="orderStatusDateFrom"
                type="date"
                value={dateFrom}
                onChange={(e) => {
            ...
            {/* To Date */}
            <div>
              <label htmlFor="orderStatusDateTo" className="block text-[11px] font-bold text-neutral-700 mb-1 uppercase tracking-wider">
                To Date
              </label>
              <input
                id="orderStatusDateTo"
                type="date"
                value={dateTo}
                onChange={(e) => {
            ...
            {/* Seller Filter */}
            <div>
              <label htmlFor="orderStatusSellerFilter" className="block text-[11px] font-bold text-neutral-700 mb-1 uppercase tracking-wider">
                Store / Seller
              </label>
              <select
                id="orderStatusSellerFilter"
            ...
            {/* Search Input */}
            <div>
              <label htmlFor="orderStatusSearchInput" className="block text-[11px] font-bold text-neutral-700 mb-1 uppercase tracking-wider">
                Search Orders
              </label>
              <div className="relative">
                <input
                  id="orderStatusSearchInput"
                  type="text"
                  value={searchQuery}
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-neutral-200/50">
            {(dateFrom || dateTo || selectedSeller !== "All Sellers" || searchQuery) && (
              <button
                type="button"
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="py-12 px-4 text-center">
                    ...
                    <p className="text-sm font-bold text-neutral-800">No {status.toLowerCase()} orders found</p>
                    <p className="text-xs text-neutral-500 mt-0.5">
                      {searchQuery || dateFrom || dateTo
                        ? "No orders match your filter criteria"
                        : `Orders in ${status.toLowerCase()} state will appear here`}
                    </p>
                  </td>
                </tr>
              ) : (
      ...
      <footer className="text-center text-xs text-neutral-400 py-3">
        HelloLocal Admin Panel • Quick-Commerce Order Operations
      </footer>
    </div>
  );
}
